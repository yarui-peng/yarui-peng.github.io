...
- Render research areas as a nested secondary-tone daisyUI card with one full-width list row per EDA4 area. Give all table header rows a distinct, bold, same-size treatment, and use the accent token for compact profile action groups so their labels remain readable in both themes.
...
- Keep lab-member publication authors as regular Arial text with CSS bold weight, without nested strong tags or padded highlight pills. In current-course cards, use the course title as the heading and show the term as a centered badge label beneath it.
