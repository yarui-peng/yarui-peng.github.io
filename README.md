...
- Research media should use the daisyUI carousel pattern for design figures: one `carousel-item w-full` slide at a time, native circular previous/next buttons, and a thin behavior adapter only when CSS scrolling needs an explicit step. Keep descriptions in image `alt` text and accessible link labels rather than adding visible captions below every image.
