...
- Research media should use compact, image-only hover galleries for design figures: one row of linked images where hover/focus expands the selected tile. Keep descriptions in image `alt` text, preserve keyboard focus states, and use a reduced-motion fallback. Do not add arrows or JavaScript when the hover gallery interaction is sufficient.
