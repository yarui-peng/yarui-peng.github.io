...
- Research media should use daisyUI's native `hover-gallery` component for design figures. Use the documented `<figure class="hover-gallery">` image structure, where horizontal pointer movement reveals the image sequence. Do not recreate its flex or hover behavior in project CSS, and do not add arrows, zoom effects, or JavaScript when the native component is sufficient.
