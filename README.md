...
- Present the theme control as `Light mode` and `Dark mode`, using sun and moon icons with accessible labels. Avoid alternate theme names that obscure the light/dark choice.
...
- Research media should use image-only daisyUI carousels for design figures. Use native carousel sizing utilities for readable large tiles (`w-5/6`, `sm:w-2/3`, `lg:w-1/2`), keep descriptions in image `alt` text and accessible link labels rather than adding visible captions below every image, and keep each gallery in one horizontally scrollable row with visible previous/next controls.
